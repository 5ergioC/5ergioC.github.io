...
function useLang() {
  const [l, set] = React.useState(_lang);
  React.useEffect(() => {
    const fn = (nl) => set(nl);
    _subs.add(fn);
...
function useTheme() {
  const [th, set] = React.useState(_theme);
  React.useEffect(() => {
    const fn = (nt) => set(nt);
    _themeSubs.add(fn);
...
function onTheme(fn) { _themeSubs.add(fn); return () => _themeSubs.delete(fn); }
...
Object.assign(window, { useLang, setLang, L, STR, LangToggle, useTheme, setTheme, getTheme, onTheme, ThemeToggle });
