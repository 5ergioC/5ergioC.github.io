export const spendant = {
  ...
  overview: [
    ...
    { en: "Spendant eliminates registration friction entirely. It intercepts notifications from Nequi, Bancolombia, and Google Pay in real time to auto-capture transactions, scans receipts via OCR, and runs a behavioral engine on-device that detects spending hotspots and fires proactive nudges before a bad habit plays out — not after. Every screen and interaction is instrumented end-to-end with Mixpanel, feeding the product decisions behind features like Habit Fixer. The architecture is offline-first, so every feature works without internet and syncs to Firestore when reconnected.",
      es: "Spendant elimina completamente la fricción de registro. Intercepta notificaciones de Nequi, Bancolombia y Google Pay en tiempo real para capturar transacciones automáticamente, escanea recibos por OCR y ejecuta un motor conductual completamente en el dispositivo que detecta hotspots de gasto y dispara alertas preventivas antes de que el mal hábito ocurra — no después. Cada pantalla e interacción está instrumentada de punta a punta con Mixpanel, analítica que alimenta decisiones de producto detrás de funciones como Habit Fixer. La arquitectura es offline-first: todo funciona sin internet y sincroniza con Firestore al reconectarse." },
    { en: "Built as a capstone project for the Mobile App Development course (ISIS3510) at Universidad de los Andes. Beyond the course's scope, the project grew a Wear OS companion app for the Google Pixel Watch, bringing voice-input expense logging and quick balance checks straight to the wrist.",
      es: "Desarrollado como proyecto final del curso de Desarrollo de Apps Móviles (ISIS3510) en la Universidad de los Andes. Por fuera del alcance del curso, el proyecto creció con una companion app para Wear OS en el Google Pixel Watch, llevando el registro de gastos por voz y la consulta rápida de saldo directo a la muñeca." },
  ],
  highlights: [
